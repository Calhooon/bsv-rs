//! BIP-39 test vectors and cross-SDK compatibility tests.
//!
//! Test vectors are from the official BIP-39 specification and the Go/TypeScript SDKs.
...
/// Official BIP-39 test vectors (from TREZOR).
/// Passphrase for all: "TREZOR"
fn test_vectors() -> Vec<TestVector> {
    vec![
        ...
        // Additional vectors with mixed patterns
        TestVector {
...
fn test_entropy_to_mnemonic() {
    for vector in test_vectors() {
        let entropy = hex::decode(vector.entropy).unwrap();
...
fn test_mnemonic_to_seed() {
    for vector in test_vectors() {
        let mnemonic = Mnemonic::from_phrase(vector.mnemonic).unwrap();
...
fn test_entropy_roundtrip() {
    for vector in test_vectors() {
        let original = hex::decode(vector.entropy).unwrap();
...
fn bad_mnemonic_sentences() -> Vec<&'static str> {
    vec![
        ...
        // Invalid word "thanks" instead of "thank"
        ...
        // Invalid word "letters" instead of "letter"
...
fn test_invalid_mnemonic_bad_checksum() {
    ...
    let phrase = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon yellow";
    assert!(
        Mnemonic::from_phrase(phrase).is_err(),
...
fn test_generate_mnemonic_15_words() {
    let mnemonic = Mnemonic::new(WordCount::Words15).unwrap();
...
fn test_generate_mnemonic_18_words() {
    let mnemonic = Mnemonic::new(WordCount::Words18).unwrap();
...
fn test_generate_mnemonic_21_words() {
    let mnemonic = Mnemonic::new(WordCount::Words21).unwrap();
...
fn test_invalid_entropy_length() {
    // 15 bytes (120 bits) - not valid
    let entropy = [0u8; 15];
    assert!(Mnemonic::from_entropy(&entropy).is_err());

    // 17 bytes (136 bits) - not valid
    let entropy = [0u8; 17];
    assert!(Mnemonic::from_entropy(&entropy).is_err());

    // 33 bytes (264 bits) - too long
    let entropy = [0u8; 33];
    assert!(Mnemonic::from_entropy(&entropy).is_err());
}

#[test]
fn test_valid_entropy_lengths() {
    // All valid entropy lengths
    for len in [16, 20, 24, 28, 32] {
        let entropy = vec![0u8; len];
        assert!(
            Mnemonic::from_entropy(&entropy).is_ok(),
            "Should accept {} byte entropy",
            len
        );
    }
}
...
fn test_to_seed_empty_passphrase() {
    ...
    assert_eq!(seed1, seed2, "Empty passphrase should match normalized");
}

#[test]
fn test_phrase_case_insensitive() {
    let phrase_lower = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
    let phrase_mixed = "Abandon ABANDON Abandon abandon ABANDON abandon ABANDON abandon abandon abandon ABANDON about";

    let mnemonic_lower = Mnemonic::from_phrase(phrase_lower).unwrap();
    let mnemonic_mixed = Mnemonic::from_phrase(phrase_mixed).unwrap();

    assert_eq!(mnemonic_lower.phrase(), mnemonic_mixed.phrase());
    assert_eq!(mnemonic_lower.entropy(), mnemonic_mixed.entropy());
}

#[test]
fn test_phrase_with_extra_spaces() {
    let phrase_normal = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
    let phrase_spaced = "  abandon   abandon  abandon abandon abandon abandon abandon abandon abandon abandon abandon   about  ";

    let mnemonic_normal = Mnemonic::from_phrase(phrase_normal).unwrap();
    let mnemonic_spaced = Mnemonic::from_phrase(phrase_spaced).unwrap();

    assert_eq!(mnemonic_normal.phrase(), mnemonic_spaced.phrase());
}
...
fn test_word_count_enum() {
    ...
    assert_eq!(WordCount::Words12.checksum_bits(), 4);
    assert_eq!(WordCount::Words15.checksum_bits(), 5);
    assert_eq!(WordCount::Words18.checksum_bits(), 6);
    assert_eq!(WordCount::Words21.checksum_bits(), 7);
    assert_eq!(WordCount::Words24.checksum_bits(), 8);
}
...
// Cross-SDK compatibility tests with zero-leading entropy
// Full set of 80 test cases from Go SDK
#[test]
fn test_zero_leading_entropy() {
    let test_cases = [
    ...
    for entropy_hex in &test_cases {
        ...
        assert_eq!(
            entropy, extracted,
            "Zero-leading entropy roundtrip failed for: {}",
            entropy_hex
        );
    }
}

// Test entropy_with_checksum() method
#[test]
fn test_entropy_with_checksum() {
    // 12-word mnemonic: 128 bits entropy + 4 bits checksum = 132 bits
    let entropy_12 = [0u8; 16];
    let mnemonic_12 = Mnemonic::from_entropy(&entropy_12).unwrap();
    let with_checksum_12 = mnemonic_12.entropy_with_checksum();
    // 132 bits = 16.5 bytes, rounded up to 17 bytes
    assert_eq!(with_checksum_12.len(), 17);
    // First 16 bytes should be the original entropy
    assert_eq!(&with_checksum_12[..16], &entropy_12);

    // 24-word mnemonic: 256 bits entropy + 8 bits checksum = 264 bits = 33 bytes
    let entropy_24 = [0u8; 32];
    let mnemonic_24 = Mnemonic::from_entropy(&entropy_24).unwrap();
    let with_checksum_24 = mnemonic_24.entropy_with_checksum();
    assert_eq!(with_checksum_24.len(), 33);
    assert_eq!(&with_checksum_24[..32], &entropy_24);
}

// Randomized entropy roundtrip tests (matching Go SDK's fuzz testing approach)
#[test]
fn test_random_entropy_roundtrip_128() {
    for _ in 0..100 {
...
#[test]
fn test_random_entropy_roundtrip_160() {
    for _ in 0..100 {
...
#[test]
fn test_random_entropy_roundtrip_192() {
    for _ in 0..100 {
...
#[test]
fn test_random_entropy_roundtrip_224() {
    for _ in 0..100 {
...
#[test]
fn test_random_entropy_roundtrip_256() {
    for _ in 0..100 {
